...
macro_rules! shader_program {
    ($display:expr, $vert_file:expr, $frag_file:expr) => ({
        ...
        let frag_src = {
            use ::std::io::Read;
            let mut buf = String::new();
            let mut f = ::std::fs::File::open(format!("src/noambition2/{}", $frag_file)).unwrap();
            f.read_to_string(&mut buf).unwrap();
...
pub fn visualizer(
    ...
    let fov_min = config
        .get("NA_FOV_MIN")
        .map(|v| v.as_float().expect("NA_FOV_MIN must be a float"))
        .unwrap_or(3.0) as f32;
    info!("NA_FOV_MIN = {}", fov_min);

    let fov_max = config
        .get("NA_FOV_MAX")
        .map(|v| v.as_float().expect("NA_FOV_MAx must be a float"))
        .unwrap_or(4.0) as f32;
    info!("NA_FOV_MAX = {}", fov_max);
    ...
    'mainloop: loop {
        ...
        let view = na::Matrix4::look_at_rh(
            &na::Point3::new(
                (time * 4.0).cos() * (beat - 0.2).max(0.0) * shake,
                -1.0,
                cam_height + (time * 4.0).sin() * (beat - 0.2).max(0.0) * shake,
            ),
        ...
        let perspective = na::Matrix4::new_perspective(
            ...
            ::std::f32::consts::PI / fov_min * (volume / 5.0) +
                ::std::f32::consts::PI / fov_max * (1.0 - (volume / 5.0)),
            0.0001,
            100.0,
        );
...
fn main() {
    ...
    framework::start("configs/noambition2.toml", visualizer);
}
